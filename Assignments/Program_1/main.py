...
class DrawGeoJson(object):
    ...
    def adjust_poly_dictionary(self):
        ...
        pp.pprint(self.adjusted_poly_dict)
    # Method for borders,rectangle and to print name of the country
    def poly(self,a,b):
        rect_latitudes = []
        rect_longitudes = []
        black=(0,0,0)
        for c,p in self.adjusted_poly_dict.items():
            temppoly = []
            for i in p:
                for j in i:
                    temppoly.append(j)
                if point_inside_polygon(a,b,temppoly):
                    # To draw border of the country
                    pygame.draw.polygon(self.screen,black,temppoly,10)
                    for k in range(len(temppoly)):
                        rect_longitudes.append(temppoly[k][0])
                        rect_latitudes.append(temppoly[k][1])
                        min_long = min(rect_longitudes)
                        max_long = max(rect_longitudes)
                        min_lat = min(rect_latitudes)
                        max_lat = max(rect_latitudes)
                        y = max_long-min_long
                        z = max_lat-min_lat
                        # To draw rectangle according to longs and lats
                    pygame.draw.rect(screen,(255,0,0),[min_long,min_lat,y,z],2)
                    pygame.init()
                        # To print name of the country
                    myfont = pygame.font.SysFont("monospace", 35, True,False)
                    label = myfont.render(c,1,(0,0,0),0)
...
if __name__ == '__main__':
    ...
    df.add_polygons(['Spain','Libya','Belgium','Germany','Ireland','Scotland','Mongolia','Germany','Egypt','Morocco','India'])
