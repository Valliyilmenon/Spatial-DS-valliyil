...
if __name__=='__main__':
    glob_data = []
    # To read all json files from 'json_condensedfiles' folder and append the data to list called 'glob_data'.
    for x in glob2.glob(DIRPATH+'/'+"**.json"):
        with open(x) as json_file:
            ...
            while i < len(data):
                ...
                i += 1
    # Writing data of all the json files to single json file called 'All_Cords.json'
    f = open(DIPATH+'/'+'All_Cords.json', 'w')
    f.write(json.dumps(glob_data,sort_keys=True,indent=4, separators=(',', ': ')))
    f.close()
    # Open our condensed json file to extract points
    f = open(DIPATH+'/'+'All_Cords.json','r')
    ...
    f = open(DIPATH+'/'+'Adjusted_All_Cords.json','w')
